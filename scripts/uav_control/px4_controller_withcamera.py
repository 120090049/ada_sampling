...
# com_flightmode1 = position #个人推测： channel
...
class Controller:
    def __init__(self):
        ...
        rospy.Subscriber("/uav1/mavros/state", State,
        ...
        rospy.Subscriber("/prometheus/sensor/monocular_down/image_raw",
                         Image, self.downlooking_camera_callback)
        rospy.Subscriber("/prometheus/sensor/monocular_front/image_raw",
                         Image, self.frontlooking_camera_callback)
        ...
        self.armServer = rospy.ServiceProxy(
            '/uav1/mavros/cmd/arming', CommandBool)
        self.setModeServer = rospy.ServiceProxy(
            '/uav1/mavros/set_mode', SetMode)
        ...
        self.pwd = sys.path[0]
        self.vw_down = cv2.VideoWriter(
            self.pwd+'/videos/down.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 10, (1280, 1280))
        self.vw_front = cv2.VideoWriter(
            self.pwd+'/videos/front.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 10, (1280, 720))
    ...
    def downlooking_camera_callback(self, data):  # e
        ...
        if (self.swithch_front):
            ...
            if (PIC_VIDEO):
                if (self.key == 'q'):
                    cv2.imwrite(self.pwd + "/pics/down" +
                                str(self.swithch_front)+".jpg", cv_image)
            else:
                if (self.swithch_front % 2 == 0):
                    # if (self.swithch_down % 2 == 0):
                    self.vw_down.write(cv_image)
                    cv2.putText(cv_image, "RECORDING", (1050, 40),
    ...
    def frontlooking_camera_callback(self, data):
        if (self.swithch_front):
            ...
            if (PIC_VIDEO):
                if (self.key == 'q'):
                    cv2.imwrite(self.pwd + "/pics/front" +
                                str(self.swithch_front)+".jpg", cv_image)
            else:
                if (self.swithch_front % 2 == 0):
...
if __name__ == "__main__":
    ...
    input = input(
        "Do you want to record video or capture image? (0 for video and 1 for picture)")
    if (input == 0):
        PIC_VIDEO = False
    elif (input == 1):
        PIC_VIDEO = True
